...
class SCryptImpl
{
    ...
    {
        FipsPBKD.Parameters params = FipsPBKD.PBKDF2.using(FipsSHS.Algorithm.SHA256_HMAC, P).withSalt(S).withIterationCount(1);
        PasswordBasedDeriver deriver = new FipsPBKD.DeriverFactory().createDeriver(params);

        return deriver.deriveKey(PasswordBasedDeriver.KeyType.CIPHER, dkLen);
    }
    ...
    private static void BlockMix(int[] B, int[] X1, int[] X2, int[] Y, int r)
    {
        ...
        for (int i = 2 * r; i > 0; --i)
        {
            ...
            Salsa20CoreEngine.salsaCore(8, X2, X1);
