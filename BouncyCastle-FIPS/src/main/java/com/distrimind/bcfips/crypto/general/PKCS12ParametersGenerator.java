    ...
    extends PBEParametersGenerator<T>
{
    ...
    public byte[] deriveKey(PasswordBasedDeriver.KeyType keyType, int keySizeInBytes)
    {
        ...
        {
        case CIPHER:
            return generateDerivedKey(KEY_MATERIAL, keySizeInBytes);
        case MAC:
            return generateDerivedKey(MAC_MATERIAL, keySizeInBytes);
        default:
    ...
    public byte[][] deriveKeyAndIV(PasswordBasedDeriver.KeyType keyType, int keySizeInBytes, int ivSizeInBytes)
    {
        ...
        {
        case CIPHER:
            rv[0] = generateDerivedKey(KEY_MATERIAL, keySizeInBytes);
            break;
        case MAC:
            rv[0] = generateDerivedKey(MAC_MATERIAL, keySizeInBytes);
            break;
        default:
            throw new IllegalStateException("Unknown type in deriveKeyAndIV: " + keyType.name());
        }

        rv[1] = generateDerivedKey(IV_MATERIAL, ivSizeInBytes);
