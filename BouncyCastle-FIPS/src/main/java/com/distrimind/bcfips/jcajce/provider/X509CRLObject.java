    ...
    extends X509CRL
{
    ...
    private final CertificateList c;
    ...
    public boolean hasUnsupportedCriticalExtension()
    {
        ...
        extns.removeAll(RFC3280CertPathUtilities.CRL_SUPPORTED_CRITICAL_EXTENSIONS);
        ...
        InvalidKeyException, NoSuchProviderException, SignatureException
    {
        ...
        try
        {
            sig = Signature.getInstance(getSigAlgName(), fipsProvider);
        }
        catch (Exception e)
        {
    ...
    public String toString()
    {
        ...
        buf.append("            Signature: ").append(
            Strings.fromByteArray(Hex.encode(sig, 0, 20))).append(nl);
        for (int i = 20; i < sig.length; i += 20)
        {
            if (i < sig.length - 20)
            {
                buf.append("                       ").append(
                    Strings.fromByteArray(Hex.encode(sig, i, 20))).append(nl);
            }
            else
            {
                buf.append("                       ").append(
                    Strings.fromByteArray(Hex.encode(sig, i, sig.length - i))).append(nl);
            }
        }
            ...
            while (e.hasMoreElements())
            {
                ...
                if (ext.getExtnValue() != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            buf.append(
                               IssuingDistributionPoint.getInstance(obj)).append(nl);
                        }
                        else if (oid
                            .equals(Extension.cRLDistributionPoints))
                        {
                            buf.append(
                                CRLDistPoint.getInstance(obj)).append(nl);
                        }
