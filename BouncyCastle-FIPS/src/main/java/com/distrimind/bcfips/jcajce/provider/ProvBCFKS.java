...
import com.distrimind.bcfips.crypto.fips.Scrypt;
import com.distrimind.bcfips.crypto.util.PBKDF2Config;
import com.distrimind.bcfips.crypto.util.PBKDFConfig;
import com.distrimind.bcfips.crypto.util.ScryptConfig;
...
import com.distrimind.bcfips.asn1.bc.EncryptedObjectStoreData;
import com.distrimind.bcfips.asn1.bc.EncryptedPrivateKeyData;
import com.distrimind.bcfips.asn1.bc.EncryptedSecretKeyData;
import com.distrimind.bcfips.asn1.bc.ObjectData;
import com.distrimind.bcfips.asn1.bc.ObjectDataSequence;
import com.distrimind.bcfips.asn1.bc.ObjectStore;
import com.distrimind.bcfips.asn1.bc.ObjectStoreData;
import com.distrimind.bcfips.asn1.bc.ObjectStoreIntegrityCheck;
import com.distrimind.bcfips.asn1.bc.PbkdMacIntegrityCheck;
import com.distrimind.bcfips.asn1.bc.SecretKeyData;
import com.distrimind.bcfips.asn1.pkcs.EncryptedPrivateKeyInfo;
import com.distrimind.bcfips.asn1.pkcs.EncryptionScheme;
...
import com.distrimind.bcfips.jcajce.BCFKSLoadStoreParameter;
import com.distrimind.bcfips.jcajce.BCLoadStoreParameter;
import com.distrimind.bcfips.jcajce.ConsistentKeyPair;
    ...
    extends AlgorithmProvider
{
        ...
        extends KeyStoreSpi
    {
        ...
        public Certificate[] engineGetCertificateChain(String alias)
        {
            ...
            if (ent != null)
            {
                ...
                {
                    EncryptedPrivateKeyData encPrivData = EncryptedPrivateKeyData.getInstance(ent.getData());
                    com.distrimind.bcfips.asn1.x509.Certificate[] certificates = encPrivData.getCertificateChain();
        ...
        private Certificate decodeCertificate(Object cert)
        {
            try
            {
                if (fipsProvider != null)
                {
                    return new X509CertificateObject(fipsProvider, com.distrimind.bcfips.asn1.x509.Certificate.getInstance(cert));
                }
                else
                {
                    try
                    {
                        ...
                        return certFact.generateCertificate(new ByteArrayInputStream(com.distrimind.bcfips.asn1.x509.Certificate.getInstance(cert).getEncoded()));
                    }
                    catch (Exception e)
                    {
                        return new X509CertificateObject(null, com.distrimind.bcfips.asn1.x509.Certificate.getInstance(cert));
                    }
                }
            }
        ...
        {
            com.distrimind.bcfips.asn1.x509.Certificate[] certChain = new com.distrimind.bcfips.asn1.x509.Certificate[chain.length];
            for (int i = 0; i != chain.length; i++)
            {
                certChain[i] = com.distrimind.bcfips.asn1.x509.Certificate.getInstance(chain[i].getEncoded());
            }
    ...
    private static final String PREFIX = "com.distrimind.bcfips.jcajce.provider.keystore" + ".bcfks.";
