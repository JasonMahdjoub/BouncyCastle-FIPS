    ...
    implements Wrapper
{
    ...
    // checksum digest
    //
    Digest sha1 = (Digest)FipsRegister.getProvider(FipsSHS.Algorithm.SHA1).createEngine();
    ...
    public void init(boolean forWrapping, CipherParameters param)
    {
        ...
        this.engine = new CBCBlockCipher((BlockCipher)FipsRegister.getProvider(FipsTripleDES.ALGORITHM).createEngine());
    ...
    public byte[] wrap(byte[] in, int inOff, int inLen)
    {
        ...
        if (inLen == 16)
        {
            keyToBeWrapped = new byte[24];

            System.arraycopy(in, inOff, keyToBeWrapped, 0, inLen);
            System.arraycopy(in, inOff, keyToBeWrapped, 16, 8);
        }
        else
        {
            keyToBeWrapped = new byte[inLen];

            System.arraycopy(in, inOff, keyToBeWrapped, 0, inLen);
        }

        DesParameters.setOddParity(keyToBeWrapped);
        ...
        throws InvalidCipherTextException
    {
        ...
        if (inLen != 40)
        {
            throw new InvalidCipherTextException("Ciphertext must be 40 bytes");
        }
