...
import com.distrimind.bcfips.crypto.internal.MacDerivationFunction;
    ...
    implements MacDerivationFunction
{
    ...
    public void init(DerivationParameters params)
    {
        ...
        this.prf.init(new KeyParameterImpl(dpiParams.getKI()));

        // --- set arguments ---

        this.fixedInputData = dpiParams.getFixedInputData();
        this.counterLocation = dpiParams.getCounterLocation();
    ...
    private void generateNext()
    {
        ...
        if (counterLocation == KDFDoublePipelineIterationParameters.AFTER_ITER || counterLocation == KDFDoublePipelineIterationParameters.AFTER_FIXED)
        {
            // --- step 5b ---
            prf.update(a, 0, a.length);
        }

        if (counterLocation == KDFDoublePipelineIterationParameters.AFTER_FIXED)
        {
            prf.update(fixedInputData, 0, fixedInputData.length);
        }
        ...
        if (counterLocation == KDFDoublePipelineIterationParameters.BEFORE_ITER)
        {
            // --- step 5b ---
            prf.update(a, 0, a.length);
        }
