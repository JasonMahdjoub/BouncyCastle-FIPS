    ...
    implements MacDerivationFunction
{
    ...
    public void init(DerivationParameters params)
    {
        ...
        this.counterLocation = feedbackParams.getCounterLocation();

        // --- init mac based PRF ---

        this.prf.init(new KeyParameterImpl(feedbackParams.getKI()));
    ...
    {
        if (counterLocation == KDFFeedbackParameters.AFTER_ITER || counterLocation == KDFFeedbackParameters.AFTER_FIXED)
        {
            if (generatedBytes == 0)
            {
        ...
        if (counterLocation == KDFFeedbackParameters.AFTER_FIXED)
        {
        ...
        if (counterLocation == KDFFeedbackParameters.BEFORE_ITER)
        {
            if (generatedBytes == 0)
            {
                prf.update(iv, 0, iv.length);
