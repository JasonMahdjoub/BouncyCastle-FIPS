...
class NewHope
{
    ...
    {
        FipsOutputDigestCalculator digCalc = new FipsSHS.OperatorFactory().createOutputDigestCalculator(FipsSHS.SHA3_256);

        UpdateOutputStream digStream = digCalc.getDigestStream();

        digStream.update(sharedKey, 0, 32);
        ...
        {
            throw new IllegalStateException("unable to finalize digest"); // should never happen
        }

        byte[] dig = digCalc.getDigest();

        System.arraycopy(dig, 0, sharedKey, 0, sharedKey.length);
    }
}
