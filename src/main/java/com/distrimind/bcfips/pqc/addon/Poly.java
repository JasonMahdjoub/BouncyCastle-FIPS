...
class Poly
{
    ...
    {
        FipsOutputXOFCalculator xofCalculator = new FipsSHS.XOFOperatorFactory().createOutputXOFCalculator(FipsSHS.SHAKE128);

        OutputStream xOut = xofCalculator.getFunctionStream();
        
        try
        {
            xOut.write(seed, 0, seed.length);

            xOut.close();
        }
        catch (IOException e)
        {
            throw new IllegalStateException("exception build XOF", e);
        }
        ...
        for (;;)
        {
            ...
            xofCalculator.getFunctionOutput(output, 0, output.length);
