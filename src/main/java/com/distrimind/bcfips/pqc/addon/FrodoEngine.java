...
class FrodoEngine
{
    ...
    private final FipsSHS.XOFOperatorFactory<FipsSHS.Parameters> digestFact;
    ...
    public FrodoEngine(int n, int D, int B, short[] cdf_table, FipsSHS.Parameters digest, FrodoMatrixGenerator mGen)
    {
        this.n = n;
        this.D = D;
        this.q = (1 << D);
        ...
        this.digestFact = new FipsSHS.XOFOperatorFactory<FipsSHS.Parameters>();
        this.digest = digest;
        this.gen = mGen;
    }
        ...
        throws IOException
    {
        ...
        OutputXOFCalculator xof = digestFact.createOutputXOFCalculator(digest);
        UpdateOutputStream dBuf = xof.getFunctionStream();
        dBuf.update(z, 0, z.length);
        dBuf.close();
        xof.getFunctionOutput(seedA, 0, seedA.length);
        ...
        xof = digestFact.createOutputXOFCalculator(digest);
        dBuf = xof.getFunctionStream();
        dBuf.update((byte)0x5f);
        dBuf.update(seedSE, 0, seedSE.length);
        dBuf.close();
        xof.getFunctionOutput(rbytes, 0, rbytes.length);

        short[] r = new short[2 * n * nbar];
        for (int i = 0; i < r.length; i++)
            r[i] = Utils.littleEndianToShort(rbytes, i * 2);
        ...
        xof = digestFact.createOutputXOFCalculator(digest);
        dBuf = xof.getFunctionStream();
        dBuf.update(pk, 0, pk.length);
        dBuf.close();
        xof.getFunctionOutput(pkh, 0, pkh.length);
        ...
        for (int i = 0; i < nbar; i++)
            for (int j = 0; j < n; j++)
                System.arraycopy(Utils.shortToLittleEndian(S_T[i*n+j]), 0,
        ...
        throws IOException
    {
        ...
        OutputXOFCalculator xof = digestFact.createOutputXOFCalculator(digest);
        UpdateOutputStream dBuf = xof.getFunctionStream();
        dBuf.update(pk, 0, len_pk_bytes);
        dBuf.close();
        xof.getFunctionOutput(pkh, 0, len_pkh_bytes);

        // 3. seedSE || k = SHAKE(pkh || mu, len_seedSE + len_k) (length in bits)
        byte[] seedSE_k = new byte[len_seedSE + len_k];
        xof = digestFact.createOutputXOFCalculator(digest);
        dBuf = xof.getFunctionStream();
        dBuf.update(pkh, 0, len_pkh_bytes);
        dBuf.update(mu, 0, len_mu_bytes);
        dBuf.close();
        xof.getFunctionOutput(seedSE_k, 0, len_seedSE_bytes + len_k_bytes);
        ...
        xof = digestFact.createOutputXOFCalculator(digest);
        dBuf = xof.getFunctionStream();
        dBuf.update(c1, 0, c1.length);
        dBuf.update(c2, 0, c2.length);
        dBuf.update(k, 0, len_k_bytes);
        dBuf.close();
        xof.getFunctionOutput(ss, 0, len_s_bytes);
    }
...
//        r = (short) ((-(short)(r >> 1) | -(short)(r & 1)) >> (8*2-1));
        ...
        throws IOException
    {
        ...
        for (int i = 0; i < nbar; i++)
            for (int j = 0; j < n; j++)
                Stransposed[i*n+j] = Utils.littleEndianToShort(Sbytes, i * n * 2 + j * 2);
        ...
        UpdateOutputStream dBuf = xof.getFunctionStream();
        dBuf.update(pkh, 0, len_pkh_bytes);
        dBuf.update(muprime, 0, len_mu_bytes);
        dBuf.close();
        xof.getFunctionOutput(seedSEprime_kprime, 0, len_seedSE_bytes + len_k_bytes);
        ...
        dBuf = xof.getFunctionStream();
        dBuf.update((byte)0x96);
        dBuf.update(seedSEprime_kprime, 0, len_seedSE_bytes);
        dBuf.close();
        xof.getFunctionOutput(rbytes, 0, rbytes.length);
        ...
        dBuf.update(c1, 0, c1.length);
        dBuf.update(c2, 0, c2.length);
        dBuf.update(kbar, 0, kbar.length);
        dBuf.close();
        xof.getFunctionOutput(ss, 0, len_ss_bytes);
    }
