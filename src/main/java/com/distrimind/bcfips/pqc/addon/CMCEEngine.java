...
class CMCEEngine
{
        ...
        throws IOException
    {
        ...
        FipsSHS.XOFOperatorFactory digestFact = new FipsSHS.XOFOperatorFactory();
        OutputXOFCalculator xof = digestFact.createOutputXOFCalculator(FipsSHS.SHAKE256);
        UpdateOutputStream dBuf = xof.getFunctionStream();
        dBuf.update((byte)64);
        dBuf.update(sk, 0, 32);
        dBuf.close();
        xof.getFunctionOutput(hash, 0, hash.length);
        ...
        throws IOException
    {
        ...
        while (true)
        {
            ...
            UpdateOutputStream dBuf = xof.getFunctionStream();
            dBuf.update(seed_a, 0, seed_a.length);
            dBuf.update(seed_b, 0, seed_b.length);
            dBuf.close();
            xof.getFunctionOutput(E, 0, E.length);
...
//            System.out.println("padding_ok: " + padding_ok);
        ...
        OutputXOFCalculator xof = digestFact.createOutputXOFCalculator(FipsSHS.SHAKE256);
        UpdateOutputStream dBuf = xof.getFunctionStream();
        dBuf.update((byte)0x02);
        dBuf.update(error_vector, 0, error_vector.length); // input
        dBuf.close();
        xof.getFunctionOutput(cipher_text, SYND_BYTES, 32);     // output
        ...
        // K = Hash((0x1 || e || C), 32)
        xof = digestFact.createOutputXOFCalculator(FipsSHS.SHAKE256);
        dBuf = xof.getFunctionStream();
        dBuf.update((byte)0x01);
        dBuf.update(error_vector, 0, error_vector.length);
        dBuf.update(cipher_text, 0, cipher_text.length); // input
        dBuf.close();
        xof.getFunctionOutput(key, 0, key.length);     // output
        ...
        throws IOException
    {
        ...
        dBuf.update(error_vector, 0, error_vector.length); // input
        dBuf.close();
        xof.getFunctionOutput(conf, 0, 32);     // output
        ...
        xof = digestFact.createOutputXOFCalculator(FipsSHS.SHAKE256);
        dBuf = xof.getFunctionStream();
        dBuf.update(preimage, 0, preimage.length); // input
        dBuf.close();
        xof.getFunctionOutput(key, 0, key.length);     // output
